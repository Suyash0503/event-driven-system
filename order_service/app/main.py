...
# ---------------------------------
# Checkout order (Mode 2 support)
# ---------------------------------
@app.post("/orders/{order_id}/checkout", response_model=OrderResponse)
def checkout_order(order_id: str):
    ...
    # simple state transition for demo
    # under process -> shipping
    new_status = "shipping" if order.get("status") == "under process" else order.get("status", "shipping")
